...
function AvalicacoesPendentes() {
  ...
  useEffect(() => {
    arrayTipo = []
    arrayIndex = []
    Axios.get(`http://10.105.75.146:24/atleta/verExamesSolicitados/${email}`)
      .then((res) => {
        setDados(res.data);
  ...
  useEffect(() => {
    if (deletarurl === true) {
      Axios.put(`http://10.105.75.146:24/atleta/modificarPdf/${email}`, {
        decisao: false,
      })
        .then((res) => res)
        .catch((error) => {
  ...
  async function ViewPdf() {
    await Axios.put(`http://10.105.75.146:24/atleta/modificarPdf/${email}`, {
      decisao: true,
    }).then((res) => {
      setPdf(res.data.msg);
    });
  ...
  function enviarExame(id_exame) {
    ...
    Axios.put(`http://10.105.75.146:24/atleta/enviarPdf/${email}`, formData,{
      headers: {
